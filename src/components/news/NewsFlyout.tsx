...
export function NewsFlyout({ item, regionName, onClose }: NewsFlyoutProps): ReactNode {
  ...
  const closeReader = (): void => setIsReading(false)
  ...
  if (isReading && canOpen) {
    // Not framable (deterministic whitelist): show the article info + image +
    // open button — never a blank iframe, no scary error message.
    if (!canEmbed) {
      return (
          ...
          <div className="flex flex-col items-center gap-4 p-6 pt-9 text-center">
            ...
            <div>
              <p className="flex items-center justify-center gap-1.5 text-xs text-slate-400">
                <IconGlobe className="h-3.5 w-3.5 text-sky-300" />
                <span className="font-semibold text-sky-300">{item.sourceName}</span> ·{' '}
                {formatRelativeTime(item.publishedAt)} · {host}
              </p>
              <h2 className="mt-1.5 text-lg leading-snug font-bold text-white">{item.title}</h2>
        ...
        <div className="flex h-full min-h-0 flex-col">
          ...
          <div className="flex items-center gap-2 py-2 pr-14 pl-4 text-xs text-slate-400">
            <span className="min-w-0 flex-1 truncate">
              {target.embeddable ? 'Lector' : 'Vista previa'} · {host}
            </span>
            <a
              href={item.link}
              target="_blank"
          ...
          <iframe
            src={target.url}
            title={item.title}
            className={`min-h-0 w-full flex-1 ${item.platform === 'youtube' || item.platform === 'telegram' ? 'bg-black' : 'bg-white'}`}
